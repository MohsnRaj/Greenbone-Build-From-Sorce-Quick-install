...
# Check if the 'gvm' user already exists
if id "gvm" &>/dev/null; then
    echo "User 'gvm' already exists. Skipping user creation."
else
    # Add the 'gvm' user with specified options
    sudo useradd -r -M -U -G sudo -s /usr/sbin/nologin gvm
fi

# Add the current user to the 'gvm' group
sudo usermod -aG gvm $USER
mkdir -p $HOME/GreenboneSource
# Set environment variables
export INSTALL_PREFIX=/usr/local
export PATH=$PATH:$INSTALL_PREFIX/sbin
export SOURCE_DIR=$HOME/GreenboneSource/source
export BUILD_DIR=$HOME/GreenboneSource/build
export INSTALL_DIR=$HOME/GreenboneSource/install
...
# Create a new sudoers file for the gvm group
echo '%gvm ALL = NOPASSWD: /usr/local/sbin/openvas' | sudo tee /etc/sudoers.d/gvm-openvas

# Set the correct permissions for the new sudoers file
sudo chmod 0440 /etc/sudoers.d/gvm-openvas

# Print a success message
echo "gvm group configuration added to sudoers."
...
# Execute the PostgreSQL commands as the postgres user
...
# Exit from the postgres user shell is not needed as the commands run in a subshell

# Create a GVM user with the name 'admin'
/usr/local/sbin/gvmd --create-user=admin

# Create a GVM user with the name 'admin' and a specified password
...
# Create ospd-openvas service file
...
# Copy ospd-openvas service file to systemd
sudo cp -v $BUILD_DIR/ospd-openvas.service /etc/systemd/system/

# Create notus-scanner service file
...
# Copy notus-scanner service file to systemd
sudo cp -v $BUILD_DIR/notus-scanner.service /etc/systemd/system/

# Create gvmd service file
...
# Copy gvmd service file to systemd
sudo cp -v $BUILD_DIR/gvmd.service /etc/systemd/system/

# Create gsad service file
